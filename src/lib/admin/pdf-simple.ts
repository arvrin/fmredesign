...
export class SimplePDFGenerator {
  ...
  private addTotals(invoice: Invoice): void {
    ...
    // Calculate position after table with proper spacing
  ...
  private addNotesAndTerms(invoice: Invoice): void {
    const pageHeight = this.doc.internal.pageSize.getHeight();
    const startY = this.tableEndY + 10;

    // More conservative check - only add new page if really needed (less than 60mm remaining)
    if (startY > pageHeight - 60) {
      this.doc.addPage();
      const newStartY = 20;
      this.addNotesAndTermsContent(invoice, newStartY);
    } else {
      this.addNotesAndTermsContent(invoice, startY);
    }
  }

  private addNotesAndTermsContent(invoice: Invoice, startY: number): void {
    let currentY = startY;
    ...
    if (invoice.notes) {
      ...
      this.doc.text(notesLines, 20, currentY + 6);
      currentY += 6 + (notesLines.length * 4) + 8;
    }

    if (invoice.terms) {
      this.doc.setFontSize(10);
      this.doc.setFont('helvetica', 'bold');
      this.doc.text('PAYMENT TERMS:', 20, currentY);

      this.doc.setFont('helvetica', 'normal');
      this.doc.setFontSize(9);
      const termsLines = this.doc.splitTextToSize(invoice.terms, 170);
      this.doc.text(termsLines, 20, currentY + 6);
      currentY += 6 + (termsLines.length * 4) + 8;
    }

    // Always add bank details section (separate from terms to avoid duplication)
  ...
  private addFooter(): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Footer text
      this.doc.text('Thank you for your business!', 105, pageHeight - 24, { align: 'center' });

      // Right side info
      this.doc.text('www.freakingminds.in', 190, pageHeight - 24, { align: 'right' });
      this.doc.text('Generated by Freaking Minds Invoice System', 190, pageHeight - 19, { align: 'right' });

      // Page number
      this.doc.text(`Page ${i} of ${pageCount}`, 105, pageHeight - 14, { align: 'center' });

      // Bottom brand color bar
      this.doc.setFillColor(179, 41, 104);
      this.doc.rect(0, pageHeight - 5, 210, 5, 'F');
    }
  }
